...
const BusinessEditAttraction = ({ navigation, route }) => {
    ...
    const EditAttraction = async () => {
        await fetch(`${url}/attractions/edit/${attraction.Attraction_Id}`, {
            method: "PUT",
            body: JSON.stringify({
                City_Id: city.City_Id, Attraction_Name: attractionName, City_Name: cityName, Address: address,
    ...
    const Checkdetails = async () => {
        ...
        if (dataCity === undefined) {
            alert('The city name is incorrect or does not exist')
            return
        }

        await EditAttraction(attraction.Attraction_Id, city.City_Id, attractionName, cityName, address, openingHour, closingHour,
